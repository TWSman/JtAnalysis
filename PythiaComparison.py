...
def main(): 
  ...
  with root_open(filename,'read') as f:
    ...
    ax.set_xscale('log')
    ax.set_yscale('log')
    for h,c,R in zip(FullJets_jT,(1,2,3,4),Rs):
      h[6].SetMarkerColor(c)
      h[6].SetMarkerStyle(24)
      ratio = h[6].Clone()
      ratio.Divide(FullJets_jT[1][6])
      ratios.append(ratio)
      rplt.errorbar(h[6],xerr=False,emptybins=False,label = 'R = {:.1f}'.format(R),axes=ax,fmt='+')    

    ax.text(0.15,0.001,'Pythia \n Full Jets\n' + r'Anti-$k_T$' + '\n' + r'$p_{{T,\mathrm{{jet}}}}$: {:02d}-{:02d} GeV'.format(60,80) ,fontsize = 10)
    ...
    for ratio,c in zip(ratios,(1,2,3,4)):
    ...
    plt.savefig("RcomparisonSignalPt6080.pdf".format(file),format='pdf') #Save figure
  
    plt.show() #Draw figure on screen

    
    drawWithErrors2Combined(FullJets_gausRMS,FullJets_gammaRMS,15,500,1,0,1.65,0,r'jet $p_T$',r'$\sqrt{\left<j_T^2\right>}$','Pythia','PythonFigures/RcomparisonRMS',separate=True)
    return
