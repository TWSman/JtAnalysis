...
class dataset(object):
  ...
  def __init__(self,name,**kwargs):
    ...
    with root_open(self._filename,'read') as f:
      self._measN = [f.Get('{}/JetPtBin/JetPtBinNFin{:02d}JetPt{:02d}'.format(self._directory,self._NFIN,i)).Integral() for i in range(self._range[0],self._range[1])] #Get number of jets by jet pT bins
      self._measBgN = [f.Get('{}/BgTrkNumberBin/BgTrkNumberBinNFin{:02d}JetPt{:02d}'.format(self._directory,self._NFIN,i)).Integral() for i in range(self._range[0],self._range[1])] #Get number of background jets
      try:
  ...
  def getJetPt(self,**kwargs):
    try:
      if('dir' in kwargs):
        ...
        print('{0[dir]}/JetPt/JetPtNFin{0[NFin]:02d}'.format({'dir':self._directory,'NFin':self._NFIN}))
        hist = self._f.Get('{0[dir]}/JetPt/JetPtNFin{0[NFin]:02d}'.format({'dir':self._directory,'NFin':self._NFIN}))
    except rootpy.io.file.DoesNotExist:
      return None
  ...
  def getHistByName(self,name,ij,**kwargs):
    ...
    try: 
      if('dir' in kwargs):
        hist = self._f.Get('{0[dir]}/{0[histname]}'.format({'dir':kwargs['dir'], 'histname':name})).Clone() #Get a single jT histogram from file
      else:
        hist = self._f.Get('{0[dir]}/{0[histname]}'.format({'dir':self._directory, 'histname':name})).Clone()  #Get a single jT histogram from file
    except rootpy.io.file.DoesNotExist:
      return None
    hist.Sumw2()
  ...
  def getHist(self,name,**kwargs):
    ...
    print(extra)
    if('dir' in kwargs):
      hist = [self._f.Get('{0[dir]}/{0[histname]}/{0[histname]}NFin{0[NFin]:02d}JetPt{0[pT]:02d}{0[extra]}'.format({'dir':kwargs['dir'], 'histname':name,'NFin':self._NFIN,'pT':i,'extra':extra})).Clone() for i in range(self._range[0],self._range[1])]  #Get jT histograms from file an array
    else:
      hist = [self._f.Get('{0[dir]}/{0[histname]}/{0[histname]}NFin{0[NFin]:02d}JetPt{0[pT]:02d}{0[extra]}'.format({'dir':self._directory, 'histname':name,'NFin':self._NFIN,'pT':i,'extra':extra})).Clone() for i in range(self._range[0],self._range[1])]  #Get jT histograms from file an array
  ...
  def getHistPta(self,name,**kwargs):
    ...
    for i in range(self._range[0],self._range[1]):
      if('dir' in kwargs):
        hist = [self._f.Get('{0[dir]}/{0[histname]}/{0[histname]}NFin{0[NFin]:02d}JetPt{0[jetpT]:02d}TrkPt{0[trkpT]:02d}'.format({'dir':kwargs['dir'], 'histname':name,'NFin':self._NFIN,'jetpT':i, 'trkpT':j})).Clone() for j in range(0,11)]  #Get jT histograms from file an array
      else:
        hist = [self._f.Get('{0[dir]}/{0[histname]}/{0[histname]}NFin{0[NFin]:02d}JetPt{0[jetpT]:02d}TrkPt{0[trkpT]:02d}'.format({'dir':self._directory, 'histname':name,'NFin':self._NFIN,'jetpT':i,'trkpT':j})).Clone() for j in range(0,11)]  #Get jT histograms from file an array
      hists.append(hist)
    #print('{0[dir]}/{0[histname]}/{0[histname]}NFin{0[NFin]:02d}JetPt{0[pT]:02d}'.format({'dir':self._directory, 'histname':name,'NFin':self._NFIN,'pT':1}))
    jetPt = [(int(re.search( r'p_{T,jet} : ([\d]*)\.[\d] - ([\d]*).[\d]*',h[0].GetTitle(), re.M|re.I).group(1)),int(re.search( r'p_{T,jet} : ([\d]*)\.[\d] - ([\d]*).[\d]*',h[0].GetTitle(), re.M|re.I).group(2))) for h in hist] #Use regular expressions to extract jet pT range from histogram titles
  ...
  def get2DHist(self,name,**kwargs):
    ...
    """
    if('dir' in kwargs):
      hist = [self._f.Get('{0[dir]}/{0[histname]}/{0[histname]}NFin{0[NFin]:02d}JetPt{0[pT]:02d}'.format({'dir':kwargs['dir'], 'histname':name,'NFin':self._NFIN,'pT':i})).Clone() for i in range(self._range[0],self._range[1])]  #Get jT histograms from file an array
    else:
      hist = [self._f.Get('{0[dir]}/{0[histname]}/{0[histname]}NFin{0[NFin]:02d}JetPt{0[pT]:02d}'.format({'dir':self._directory, 'histname':name,'NFin':self._NFIN,'pT':i})).Clone() for i in range(self._range[0],self._range[1])]  #Get jT histograms from file an array
    jetPt = [(int(re.search( r'p_{T,jet} : ([\d]*)\.[\d] - ([\d]*).[\d]*',h.GetTitle(), re.M|re.I).group(1)),int(re.search( r'p_{T,jet} : ([\d]*)\.[\d] - ([\d]*).[\d]*',h.GetTitle(), re.M|re.I).group(2))) for h in hist] #Use regular expressions to extract jet pT range from histogram titles
    if(kwargs.get('jetpt',False)):
      return hist,jetPt
    else:
  ...
  def getGraphs2(self,start,end,**kwargs):
    if(not self._fitDone):
      ...
      self._fits = []
      self._parameters = []
      jt,jetpt = self.getSubtracted('JetConeJtWeightBin','BgJtWeightBin',jetpt=True)
      print(jetpt)
      ...
      for h,he,g in zip((self._gausYield,self._gammaYield),(self._gausYielde,self._gammaYielde),(self._gausYieldg,self._gammaYieldg)):
        for x,xe,a,e,i in zip(jetPtCenter[2:],jetPtErrors[2:],h[2:],he[2:],range(len(self._gausRMS)-2)):
          g.SetPoint(i,x,a)
          g.SetPointError(i,xe,xe,e,e)
      
      for a,b,c,d,e,f,i in zip(self._gausRMS[2:],self._gammaRMS[2:],self._gausRMSe[2:],self._gammaRMSe[2:],jetPtCenter[2:],jetPtErrors[2:],range(len(self._gausRMS)-2)):
      ...
      for a,b,c,d,e,f,i in zip(self._gausYield[2:],self._gammaYield[2:],self._gausYielde[2:],self._gammaYielde[2:],jetPtCenter[2:],jetPtErrors[2:],range(len(self._gausYield)-2)):
...
class datasetMixed(dataset,object):
  ...
  def __init__(self,name,**kwargs):
    ...
    self._measBgN2 = [self._f2.Get('{}/BgTrkNumberBin/BgTrkNumberBinNFin{:02d}JetPt{:02d}'.format(self._directory2,self._NFIN,i)).Integral() for i in range(self._range[0],9)] #Get number of background jets
    self._measRndmBgN1 = [self._f.Get('{}/hNumber/hNumberNFin{:02d}'.format(self._directory1,self._NFIN)).GetBinContent(7+i) for i in range(self._range[0],self._range[1])] #Get number of background jets
    self._measRndmBgN2 = [self._f2.Get('{}/hNumber/hNumberNFin{:02d}'.format(self._directory2,self._NFIN)).GetBinContent(7+i) for i in range(self._range[0],9)] #Get number of background jets
...
def compareHistsWithRatio(dataset,histnames,labels,step=2,start=1,extras=None):
  ...
  if(extras is None):
    ...
    print('Extras is ')
    print(extras)
    hists = [dataset.getHist(histname,jetpt = True,extra=extra) for histname,extra in zip(histnames,extras)]
  fig, axs = defs.makegrid(4,2,xlog=True,ylog=True,d=d,shareY = False)
  axs = axs.reshape(8)
  axs[1].text(0.2,0.005,d['system'] +'\n'+  d['jettype'] +'\n'+ d['jetalg'] + '\n'+ d['trigger'],fontsize = 7)
  colors = [1,2,3,4,5]
  ...
  for hist in hists:  #loop over histograms to be compared
    ratios = []
    #divider = hists[0][0]
    for s,divider in zip(hist[0],hists[0][0]): #hist[0] is list of histograms, (hist is a tuple with histograms + jetpT bins)
      h = s.Clone()
      h.Divide(divider)
      ratios.append(h)
  ...
  for i,set in enumerate(ratiosets[1:]):
    for ratio,pT,ax,j in zip(set[0][1::step],set[1][1::step],axs[4:9],range(0,5)):
      rplt.errorbar(ratio,xerr=False,emptybins=False,axes=ax,fmt='o') #Plot ratio histogram, 
      #if(i == 0):
      ax.set_xlim([0.1,20]) #Set x-axis limits
...
def JtWithBackgroundRatio(dataset,histname,bgname,rndmBgname="",**kwargs):
  ...
  fig, axs = defs.makegrid(4,2,xlog=True,ylog=True,d=d,shareY = False)
  axs = axs.reshape(8)
  axs[1].text(1.2,10,d['system'] +'\n'+  d['jettype'] +'\n'+ d['jetalg'] + '\n'+ d['trigger'] + '\n' + dataset.name(),fontsize = 7)
  
  for jT,bgJt,rndmbg,pT,ax,j in zip(jtHistos[0][start::step],bgHistos[0][start::step],bgRndmHisto[0][start::step],jtHistos[1][start::step],axs[0:4],range(0,5)):
    rplt.errorbar(jT,xerr=False,emptybins=False,axes=ax,label='Inclusive Jt',fmt='o') #Plot jT histogram,
    rplt.errorbar(bgJt,xerr=False,emptybins=False,axes=ax,label='Perp. cone Bg',fmt='o') #Plot jT histogram,
    if(kwargs.get('includeRandom',False)):
      rplt.errorbar(rndmbg,xerr=False,emptybins=False,axes=ax,label='Random Bg',fmt='o') #Plot jT histogram,
    ax.set_xlim([0.1,20]) #Set x-axis limits
    ax.set_ylim([5e-5,4e3]) #Set y-axis limits
    ax.text(0.15,5e-4,r'$p_{{T,\mathrm{{jet}}}}$:''\n'r' {:02d}-{:02d} GeV'.format(pT[0],pT[1])) 
  ...
  axs[4].set_ylabel('Ratio \n Background/Inclusive',fontsize=9) #Add y-axis labels to left- and righmost subfigures
  axs[-1].set_ylabel('Ratio \n Background/Inclusive',fontsize=9)
  if(kwargs.get('includeRandom',False)):
    for ratio,pT,ax,j in zip(ratios2[1::2],jtHistos[1][1::2],axs[4:9],range(0,5)):
...
def fitJtHisto(histo,method,cut,iJet,iFinder,title="",drawFit=False):
  ...
  if method == "alt":
    ...
    peak = beta/(alpha+1)
    peake = math.sqrt(pow(betae/(alpha+1),2)+pow(alphae*beta/pow(alpha+1,2),2))
    
  gammaYield = constant * beta /(alpha - 1)
  gammaRMS = beta /math.sqrt((alpha-2)*(alpha-3))
  gammaYielde =math.sqrt(pow(beta *  constante/(alpha-1),2) + pow(constant*beta*alphae/pow(alpha-1,2),2) + pow(constant * betae/(alpha-1),2))
  gammaRMSe = math.sqrt(pow((5-2*alpha)*beta*alphae/pow(2*((alpha-2)*(alpha-3)),1.5),2) + pow(betae/math.sqrt((alpha-2)*(alpha-3)),2))
