...
def main(): 
  ...
  with root_open(filename,'read') as fin:
    ...
    for iF in (6,7,8):
      #iF = 8
      c = 1
      jT = [fin.get('AliJJetJtTask/AliJJetJtHistManager/JetConeJtWeightBin/JetConeJtWeightBinNFin{:02d}JetPt{:02d}'.format(iF,ij)) for ij in range(Njets)]
      bgJt = [fin.get('AliJJetJtTask/AliJJetJtHistManager/BgJtWeightBin/BgJtWeightBinNFin{:02d}JetPt{:02d}'.format(iF,ij)) for ij in range(Njets)]
      jetPtBin = [fin.get('AliJJetJtTask/AliJJetJtHistManager/JetPtBin/JetPtBinNFin{:02d}JetPt{:02d}'.format(iF,ij)) for ij in range(Njets)]
      nJets = [h.Integral() for h in jetPtBin]
      nBgs = [fin.get('AliJJetJtTask/AliJJetJtHistManager/BgTrkNumberBin/BgTrkNumberBinNFin{:02d}JetPt{:02d}'.format(iF,ij)).Integral() for ij in range(Njets)]
    ...
    print(len(FullJets_gausRMS))
    drawWithErrors2Combined(FullJets_gausRMS,FullJets_gammaRMS,15,500,1,0,1.65,0,r'$p_\mathrm{T,jet}$',r'$\sqrt{\left<j_\mathrm{T}^2\right>}$','Pythia Truth','PythonFigures/RcomparisonRMS',)
    drawWithErrors2Combined(FullJets_gausYield,FullJets_gammaYield,15,500,1,0,10,0,r'$p_\mathrm{T,jet}$',r'Yield','Pythia Truth','PythonFigures/RcomparisonYield',)
    ...
    axs[1].text(0.02,0.005,'Pythia\n' r'pPb $\sqrt{s_{NN}} = 5.02 \mathrm{TeV}$' '\n Full jets \n' r'Anti-$k_\mathrm{T}$' ,fontsize=7) #Add text to second subfigure, first parameters are coordinates in the drawn scale/units
    for hists,R in zip(FullJets_jT,Rs):
      for jT,ax,i,pT in zip(hists[2:],axs[0:4],range(0,9),jetPt[2:]):
        ...
        ax.text(0.3,1e2,r'$p_\mathrm{T,jet}$:''\n'r' {:02d}-{:02d} GeV'.format(pT[0],pT[1])) 
...
def drawWithErrors2Combined(hists,hists2,xlow,xhigh,logx,ylow,yhigh,ylog,xTitle,yTitle,title,file):
  ...
  for h,h2,c,R in zip(hists,hists2,(1,2,3,4),Rs):
    ...
    if c == 1:
      rplt.errorbar(h,xerr=False,emptybins=False,label = 'Narrow',axes=ax,fmt='+')
    else:
      rplt.errorbar(h,xerr=False,emptybins=False,axes=ax,fmt='+')
    #rplt.errorbar(h,xerr=False,emptybins=False,axes=ax,fmt='+')
 
    if(R > 0):
      rplt.errorbar(h2,xerr=False,emptybins=False,axes=ax2,label='R = {:.1f} Wide'.format(R),fmt='+')
    else: 
      rplt.errorbar(h2,xerr=False,emptybins=False,axes=ax2,label='R = {:.1f} Data'.format(-R),fmt='+')

  
  print("({} - {})/9.0 + {} is {}".format(yhigh,ylow,ylow,(yhigh-ylow)/9.0+ylow))
  ax.text(100,(yhigh-ylow)/3.0+ylow,title + '\n' + d['system'] +'\n'+  d['jettype'] + '\n' + r'Anti-$k_\mathrm{T}$',
          fontsize = 10)
